...
char* *szBmpResources = NULL;

//-----------------------------------------------------------------------------
void DDSetResourceTable(char** _pszResTab)
{
	szBmpResources = _pszResTab;
}
...
DDLoadBitmap2(IDirectDraw4 * pdd, LPCSTR szBitmap, int dx, int dy)
{
    HBITMAP                 hbm;
    BITMAP                  bm;
    DDSURFACEDESC2          ddsd;
...
extern "C" IDirectDrawSurface4 *
DDLoadBitmap(IDirectDraw4 *pdd, DWORD dwResID, int dx, int dy)
{
	char szStr[1024];
	sprintf(szStr,"%s.bmp",szBmpResources[dwResID]);
	return ( DDLoadBitmap2(pdd,szStr,dx,dy) );
}
...
//-----------------------------------------------------------------------------
HRESULT
DDReLoadBitmap2(IDirectDrawSurface4 * pdds, LPCSTR szBitmap)
{
    HBITMAP                 hbm;
    HRESULT                 hr;
...
HRESULT
DDReLoadBitmap(IDirectDrawSurface4 * pdds, DWORD dwResID)
{
	return ( DDReLoadBitmap2(pdds,szBmpResources[dwResID]) );
}
...
extern "C" COLORREF 
DDGetPixel(IDirectDrawSurface4 *pdds, int x,int y)
{
	/*
	COLORREF                rgbT = 0;
	HDC                     hdc;
    ...
    {
        rgbT = GetPixel(hdc, x, y);     // Save current pixel value
        pdds->ReleaseDC(hdc);
    }
	...
	*/
    ...
    if (hres == DD_OK)
    {
        dw = *((DWORD *)  ((char*)ddsd.lpSurface + ddsd.lPitch*y + x*4));
    }
